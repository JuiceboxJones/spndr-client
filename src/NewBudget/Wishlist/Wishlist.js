    ...
    const handleList = this.state.wishlist.map((item, index) => {
      return (
        <li key={item.id || index}>
          <a href={item.url}>{item.name}</a> <span id="wishlist_spacing" />$
          {item.price}{' '}
          <button
            type="button"
            id="delete"
          ...
          >
            ️
            <span id="delete" role="img" aria-label="delete">
              ❌
            </span>
          </button>
          <p />
    ...
    return (
      <div className="wishlist_page_container">
        ...
        <div className="wishlist_form_container">
          <form
            className="wishlist_form"
            onSubmit={e => this.handleAddToWishlist(e)}
          >
            <div className="wishlist_input_container">
              ...
              <input
                ...
                placeholder="100.00"
              />
              <button type="submit" id="add_wishlist">
                Add to Wishlist
              </button>
            </div>
          </form>
