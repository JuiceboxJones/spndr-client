...
class Summary extends Component {
  ...
  state = { 
   ...
   componentDidMount(){
    ...
    handleExpensesTotal(exp) {
      ...
      this.setState({ expenseTotal: sum }, () => this.handleCurrentAccountBal());
      }
    }
  ...
  render() { 
    ...
    const handleWishlist = this.state.wishlist.map(wishes => {
      return (
        <li key={wishes.id}>
          <a href={wishes.url}>{wishes.name}</a> <span>${wishes.price}</span> <span>Buy: {this.handleWishlistPurchase(wishes.price)}</span>
          <p />
        </li>
      );
    });
